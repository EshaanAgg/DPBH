...
const getElementArea = function (element) {
	const rect = element.getBoundingClientRect();
	return rect.height * rect.width;
};

const getClientRect = function (element) {
...
const getBackgroundColor = function (element) {
...
const getRandomSubarray = function (arr, size) {
...
const elementCombinations = function (arguments) {
	const r = [],
		arg = arguments,
		max = arg.length - 1;

	function helper(arr, i) {
		for (const j = 0, l = arg[i].length; j < l; j++) {
			const a = arr.slice(0);
			a.push(arg[i][j]);
			if (i === max) {
				r.push(a);
...
const getVisibleChildren = function (element) {
	if (element) {
...
const isShown = function (element) {
	const displayed = function (element, style) {
		if (!style) {
	...
	const getOpacity = function (element, style) {
		if (!style) {
			style = window.getComputedStyle(element);
		}
	...
	const positiveSize = function (element, style) {
		if (!style) {
			style = window.getComputedStyle(element);
		}
	...
	const getOverflowState = function (element) {
		...
		function getOverflowParent(e) {
			...
			function canBeOverflowed(container) {
				...
				if (containerDisplay.startsWith("inline")) return false;
				if (position === "absolute" && style.position === "static") return false;
	...
	function hiddenByOverflow(element) {
		return (
			...
			Array.from(element.childNodes).every(
				(n) => n.nodeType !== Node.ELEMENT_NODE || hiddenByOverflow(n) || !positiveSize(n)
			)
		);
	}
	...
	if (tagName === "input" && element.type.toLowerCase() === "hidden") return false;
	if (tagName === "noscript" || tagName === "script" || tagName === "style") return false;

	const style = window.getComputedStyle(element);
	if (style == null) return false;
	if (style.visibility === "hidden" || style.visibility === "collapse") return false;

	if (!displayed(element, style)) return false;
	if (getOpacity(element, style) === 0.0) return false;
	if (!positiveSize(element, style)) return false;
...
const isInteractable = function (element) {
	function isEnabled(element) {
		...
		if (!disabledSupportElements.includes(tagName)) return true;
...
const containsTextNodes = function (element) {
	if (element) {
...
const isPixel = function (element) {
	const rect = element.getBoundingClientRect();
...
const containsBlockElements = function (element, visibility = true) {
	for (const be of blockElements) {
		const children = Array.from(element.getElementsByTagName(be));
		if (visibility)
			for (child of children)
				if (isShown(child)) return true;
				else return children.length > 0 ? true : false;
	}

	return false;
};

const isWhitespace = function (element) {
